...
def init_csv(csv_path: Path) -> bool:
    ...
    if not csv_exists:
        with open(csv_path, "w", newline="", encoding="utf-8") as cf:
            ...
            writer.writerow([
                "id", "date_iso", "from_id", "from_name", "text",
                "views", "forwards", "replies", "media_type", "media_path", "link"
            ])
    return csv_exists


async def download_media(message, media_dir: Path) -> Tuple[Optional[str], Optional[str]]:
    """Download media from message if available."""
    if not message.media:
        return None, None
    
    if not (isinstance(message.media, MessageMediaPhoto) or 
...
class CSVWriter:
    ...
    def _find_latest_split_file(self):
        ...
        if existing_files:
            ...
            self.current_file_count = self._count_rows_in_file(latest_path) - 1  # Subtract header
...
def extract_message_data(message, entity, media_type: Optional[str], media_path: Optional[str]) -> dict:
    """Extract relevant data from a Telegram message."""
    row = {
        "id": message.id,
        "date_iso": message.date.isoformat() if message.date else None,
        "from_id": getattr(getattr(message, "from_id", None), "user_id", None) or 
                   getattr(message, "sender_id", None),
        "from_name": None,
        "text": message.message or (message.raw_text if hasattr(message, "raw_text") else None),
        "views": getattr(message, "views", None),
        "forwards": getattr(message, "forwards", None),
        "replies": getattr(getattr(message, "replies", None), "replies", None),
        "media_type": media_type,
        "media_path": media_path,
        "link": f"https://t.me/{entity.username}/{message.id}" if getattr(entity, "username", None) else None
    }
    ...
    try:
        if message.sender:
            row["from_name"] = (
                ...
                f"{getattr(message.sender, 'first_name', '')} {getattr(message.sender, 'last_name', '')}".strip() or 
                None
            )
    except Exception:
        pass
        ...
        with open(jsonl_path, "a", encoding="utf-8") as jf:
            for msg in batch:
                ...
                csv_writer_manager.writerow([
                    row["id"], row["date_iso"], row["from_id"], row["from_name"],
                    (row["text"] or "").replace("\n", "\\n"),
                    row["views"], row["forwards"], row["replies"],
                    row["media_type"], row["media_path"], row["link"]
                ])
...
"""
    ...
    parser.add_argument(
        "channel",
        help="Channel to export (@username or t.me/slug)"
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
